...
export default function WatchlistMenu({
  ...
  return (
    <div
      ref={menuRef}
      className="absolute top-full right-0 mt-1 w-64 dark:bg-[#1a1a1a] bg-white rounded-lg shadow-2xl border dark:border-white/10 border-gray-200 overflow-hidden z-50 flex flex-col max-h-80"
    >
      {/* Header */}
        ...
        <>
          <div className="flex-1 overflow-y-auto min-h-0">
            {lists.length === 0 ? (
              ...
              lists.map((list) => {
                const isInList = stockInLists[list.id];
                return (
                  <button
                    key={list.id}
                    onClick={() => onAddToList(symbol, list.id)}
                    className="w-full px-4 py-2.5 text-left flex items-center justify-between hover:dark:bg-white/5 hover:bg-gray-50 transition-colors text-sm"
                  >
                    <span className={`${isInList ? 'dark:text-cyan-400 text-blue-600 font-medium' : 'dark:text-gray-300 text-gray-700'}`}>
                      {list.name}
                    </span>
                    {isInList && (
          ...
          {/* Create New List - Fixed at bottom */}
          <button
